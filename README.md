...
学程记录是一个微信小程序，帮助苏州家长和学生查询学校与历史公开录取分数线、记录考试总分，并把当前总分与手动选择的目标学校做数字对照。公开数据由后台发布并通过远程链路更新；用户成绩、目标学校和档案只保存在本机。历史数据仅供了解，不代表未来录取结果。
...
小程序个人数据不接入登录、手机号、openid、unionid、云开发、Supabase、AI、支付、广告、定位、地图 SDK、推送、统计 SDK 或云同步；公开数据仅通过 `https://api.royalcup.top` 读取后台发布内容，不上传任何用户数据。
...
- 远程公开数据：1.2.0 起通过 `https://api.royalcup.top`（request 合法域名）发布数据 Release；断网时使用 last-known-good 本机缓存，首次无缓存时使用代码包数据。
