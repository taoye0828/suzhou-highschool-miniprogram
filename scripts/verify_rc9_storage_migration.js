...
assert.strictEqual(migrated.toVersion, 4)
assert.deepStrictEqual(migrated.applied, ['v1 → v2', 'v2 → v3', 'v3 → v4'])
assert.strictEqual(storage.STORAGE_SCHEMA_VERSION, 4)
assert.strictEqual(storage.getProfiles().length, 1)
assert.strictEqual(storage.getScoreRecords().length, 1)
assert.strictEqual(storage.getScoreRecords()[0].unknownField, 'must-stay')
...
assert.strictEqual(currentMigration.state.version, 4)
